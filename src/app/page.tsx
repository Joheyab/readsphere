    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-32">
          ...
          <p className="text-zinc-600 text-sm mt-1">Sigue a otros usuarios para ver sus reseñas.</p>
        </div>
      ) : (
        feed.map((item, i) => {
          if (item.type === "review") return <ReviewCard key={i} item={item} router={router} />;
          if (item.type === "book") return <BookCard key={i} item={item} router={router} />;
          if (item.type === "person") return <PersonCard key={i} item={item} router={router} />;
          if (item.type === "activity") return <ActivityCard key={i} item={item} router={router} />;
          return null;
        })
      )}
    </div>
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={() => router.push(`/profile/${data.profiles?.username}`)} className="flex items-center gap-2 hover:opacity-80 transition">
          ...
          <div className="text-left">
            <p className="text-white text-sm font-medium">@{data.profiles?.username}</p>
            <p className="text-zinc-600 text-xs">{new Date(data.created_at).toLocaleDateString("es-CR")}</p>
          </div>
        </button>
        {isFollowing && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-violet-600/20 text-violet-400 border border-violet-600/30">Siguiendo</span>
        )}
      </div>

      {data.books && (
        <button onClick={() => router.push(`/books/${data.books.id}`)} className="flex items-center gap-3 w-full text-left hover:opacity-80 transition">
          <div className="w-10 h-14 rounded-lg overflow-hidden bg-zinc-800 flex-shrink-0">
            {data.books.cover_url ? (
              <img src={data.books.cover_url} alt="" className="w-full h-full object-cover" />
            ) : (
          ...
          <div>
            ...
            {data.books.authors && <p className="text-zinc-500 text-xs">{data.books.authors.name}</p>}
          </div>
        </button>
      )}

      <div className="space-y-1">
        <span className="text-amber-400">{"★".repeat(data.rating)}{"☆".repeat(5 - data.rating)}</span>
        {data.content && <p className="text-zinc-300 text-sm leading-relaxed">{data.content}</p>}
      </div>
    </div>
  );
}
...
function BookCard({ item, router }: { item: FeedItem; router: any }) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
      ...
      <button onClick={() => router.push(`/books/${data.id}`)} className="flex items-center gap-3 w-full text-left hover:opacity-80 transition">
        ...
        <div className="flex-1">
          ...
          {data.authors && <p className="text-zinc-500 text-xs mt-0.5">{data.authors.name}</p>}
          {data.book_genres?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {data.book_genres.slice(0, 2).map((bg: any, i: number) => bg.genres && (
                <span key={i} className="px-2 py-0.5 text-xs rounded-md bg-zinc-800 text-zinc-400">{bg.genres.name}</span>
              ))}
            </div>
          )}
        </div>
...
function PersonCard({ item, router }: { item: FeedItem; router: any }) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => router.push(`/profile/${data.username}`)} className="flex items-center gap-3 hover:opacity-80 transition">
          ...
          <div className="flex flex-col text-left">
            <p className="text-white text-sm font-medium">@{data.username}</p>
            {data.favorite_genres?.length > 0 && (
              <p className="text-zinc-500 text-xs mt-0.5">{data.favorite_genres.slice(0, 3).join(", ")}</p>
            )}
          </div>
        </button>
        <button
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-3">
      <button onClick={() => router.push(`/profile/${data.profiles?.username}`)} className="flex items-center gap-2 hover:opacity-80 transition">
        ...
        <p className="text-zinc-400 text-sm">
          <span className="text-white font-medium">@{data.profiles?.username}</span>{" "}
